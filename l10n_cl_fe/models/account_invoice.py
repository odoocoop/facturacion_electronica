            ...
            if line.invoice_line_tax_ids:
                for t in line.invoice_line_tax_ids:
                    if t.uom_id and t.uom_id.category_id != line.uom_id.category_id:
                        raise UserError(
                            "Con este tipo de impuesto, solamente deben ir unidades de medida de la categoría %s"
                            % t.uom_id.category_id.name
                        )
                    if t.mepco:
    ...
    def get_taxes_values(self):
        ...
        for line in self.invoice_line_ids:
            if not line.account_id:
                continue
            if (
                line.invoice_line_tax_ids and line.invoice_line_tax_ids[0].price_include
            ):  # se asume todos losproductos vienen con precio incluido o no ( no hay mixes)
                if (
                    included or not tax_grouped
                ):  # genero error en caso de contenido mixto, en caso primer impusto no incluido segundo impuesto incluido
                    for t in line.invoice_line_tax_ids:
                        if t not in totales:
                            totales[t] = 0
            ...
            if (totales and not included) or (included and not totales):
                raise UserError(
                    "No se puede hacer timbrado mixto, todos los impuestos en este pedido deben ser uno de estos dos:  1.- precio incluído, 2.-  precio sin incluir"
                )
            taxes = line.invoice_line_tax_ids.compute_all(
                line.price_unit,
                self.currency_id,
            ...
            for t in line.invoice_line_tax_ids:
                ...
                if t.amount == 0 or t.sii_code in [0]:  # @TODO mejor manera de identificar exento de afecto
                    ...
                    lines["Impuesto"].append(
                        {"CodImp": t.sii_code, "price_include": taxInclude, "TasaImp": amount,}
                    )
