...
class SIIResumenBoletaVoucher(models.Model):
    ...
    def set_movimientos(self):
        ...
        query = [
            ...
            ('reconciled_line_ids.invoice_id.date_invoice' , '>=', current.strftime('%Y-%m-%d')),
            ('reconciled_line_ids.invoice_id.date_invoice' , '<=', next_month.strftime('%Y-%m-%d')),
    ...
    def _id_doc(self):
        return {
            ...
            'FchEmis': self.fecha_emision,
            'MntBruto': 1,
            'FmaPago': 1,
            'FchVenc': self.fecha_emision,
        }
